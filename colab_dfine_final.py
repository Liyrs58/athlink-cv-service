#!/usr/bin/env python3
"""
D-FINE + Sports ReID — FINAL COLAB CELL
Uses transformers.pipeline API (no auth headaches)
...
subprocess.run([sys.executable, "-m", "pip", "-q", "install",
                "ultralytics", "boxmot", "transformers>=4.40", "accelerate"], check=True)
print("✓ Dependencies installed")

# ── 4. Download D-FINE model via pipeline (auto-handles caching) ──────────────
...
# Load D-FINE detector using transformers.pipeline (handles auth/caching internally)
dfine_detector = hf_pipeline("object-detection", model="rudrasinghm/dfine-football-detector")
print("✓ D-FINE loaded")

# Get class labels from model config
dfine_model = dfine_detector.model
dfine_processor = dfine_detector.image_processor

# Head Override Workaround to map intermediate prediction heads to head 0
dfine_model.class_embed[1] = dfine_model.class_embed[0]
dfine_model.class_embed[2] = dfine_model.class_embed[0]
dfine_model.bbox_embed[1] = dfine_model.bbox_embed[0]
dfine_model.bbox_embed[2] = dfine_model.bbox_embed[0]

dfine_model.model.decoder.class_embed[1] = dfine_model.model.decoder.class_embed[0]
dfine_model.model.decoder.class_embed[2] = dfine_model.model.decoder.class_embed[0]
dfine_model.model.decoder.bbox_embed[1] = dfine_model.model.decoder.bbox_embed[0]
dfine_model.model.decoder.bbox_embed[2] = dfine_model.model.decoder.bbox_embed[0]

_id2label = dfine_model.config.id2label
...
# ── 7. Define D-FINE detection wrapper ─────────────────────────────────────────
sys.path.insert(0, str(REPO))

_debug_dfine = [True, 0]  # [print_flag, frame_count]
...
def _dfine_detect(frame_bgr: np.ndarray, conf_threshold: float = 0.35):
    ...
    # Convert BGR numpy array to PIL Image (pipeline expects PIL)
    frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
    pil_img = PILImage.fromarray(frame_rgb)
    
    device = next(dfine_model.parameters()).device
    inputs = dfine_processor(images=pil_img, return_tensors="pt").to(device)
    ...
    if _debug_dfine[0]:
        print(f"\n[D-FINE DEBUG] Frame {_debug_dfine[1]}, {h}x{w}")
        print(f"  After filter: {len(dets)} dets")
        _debug_dfine[0] = False
