...
# IMPROVEMENT 1: Use fine-tuned football model if available, fall back to generic [2026-03-25]
_FOOTBALL_MODEL = os.path.join(os.path.dirname(__file__), '..', 'models', 'football_yolo11.pt')
...
def _interpolate_ball_positions(ball_trajectory: list) -> list:
    """
    IMPROVEMENT 3: Fill gaps in ball detection using linear interpolation. [2026-03-25]
    Makes ball tracking smooth and continuous instead of flickering.
    Only interpolates gaps of up to 15 frames (~0.5s at 30fps).
    """
    ...
    try:
        ...
        # Build detected frame -> (x, y) mapping
        frame_to_pos = {
            item['frameIndex']: (item['x'], item['y'])
            for item in ball_trajectory
            if item.get('confidence', 0) > 0 and 'x' in item and 'y' in item
        }

        if not frame_to_pos:
            return ball_trajectory

        min_frame = min(frame_to_pos)
        max_frame = max(frame_to_pos)
        all_frames = list(range(min_frame, max_frame + 1))

        df = pd.DataFrame({
            'frame': all_frames,
            'x': [frame_to_pos.get(f, (None, None))[0] for f in all_frames],
            'y': [frame_to_pos.get(f, (None, None))[1] for f in all_frames],
        })
        df['x'] = pd.to_numeric(df['x'], errors='coerce')
        df['y'] = pd.to_numeric(df['y'], errors='coerce')
        df['x'] = df['x'].interpolate(method='linear', limit=15)
        df['y'] = df['y'].interpolate(method='linear', limit=15)
        ...
        existing_frames = {item['frameIndex'] for item in ball_trajectory}
        result = list(ball_trajectory)
        for frame_idx, (x, y) in interpolated.items():
        ...
        result.sort(key=lambda item: item['frameIndex'])
        logger.info("Ball interpolation: %d detected → %d total positions", len(ball_trajectory), len(result))
        return result

    except Exception as e:
        logger.warning("Ball interpolation failed: %s — using raw trajectory", e)
        return ball_trajectory
...
def run_tracking(
    ...
    # IMPROVEMENT 3: Ball position interpolation [2026-03-25]
    ball_trajectory = _interpolate_ball_positions(ball_trajectory)
